...
from mercearia.usecases.produto.get_all_produtos import GetAllProdutos # Ajustado o path
...
class TestGetAllProdutos:
    ...
    async def test_execute_returns_all_produtos(self, mock_produto_repository: AsyncMock):
        """
        ...
        expected_produtos = [
            Produto(id="prod1", nome="Arroz", descricao="Arroz agulhinha 5kg", preco=25.00, imagem="url1"),
            Produto(id="prod2", nome="Feijão", descricao="Feijão carioca 1kg", preco=8.50, imagem="url2"),
            Produto(id="prod3", nome="Leite", descricao="Leite integral 1L", preco=5.00, imagem="url3"),
        ]
        ...
        get_all_produtos_use_case = GetAllProdutos(produto_repository=mock_produto_repository)
    ...
    @pytest.mark.asyncio
    async def test_execute_returns_empty_list_if_no_produtos(self, mock_produto_repository: AsyncMock):
        """
        Testa se o caso de uso GetAllProdutos retorna uma lista vazia
        quando o repositório não tem produtos.
        """
        # Arrange
        mock_produto_repository.get_all.return_value = [] # Repositório retorna lista vazia
        get_all_produtos_use_case = GetAllProdutos(produto_repository=mock_produto_repository)
