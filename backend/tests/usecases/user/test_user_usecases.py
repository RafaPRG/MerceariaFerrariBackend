...
from mercearia.domain.value_objects.password_vo import (
    Password,
    PasswordValidationError,
)  # Adicionado PasswordValidationError
from mercearia.domain.entities.user import User
from mercearia.domain.repositories.user_repository import (
    UserRepository,
)  # Para o spec do mock
...
class TestLoginUser:
    ...
    async def test_login_success(self, mock_user_repository: AsyncMock):
        ...
        assert (
            called_email_vo.value() == test_email_str
        )  # Verifica o valor da string do Email VO
    ...
    async def test_login_failure_invalid_credentials(
        ...
        assert isinstance(called_password_vo, Password)
        assert (
            called_password_vo.verify(str(Password(test_wrong_password_str))) is True
        )  # A senha é válida para o VO, mas o login falha no repositório
...
class TestUpdatePassword:
    ...
    async def test_update_password_success(self, mock_user_repository: AsyncMock):
        ...
        assert (
            called_email_vo.value() == test_email_str
        )  # <-- CORREÇÃO: Comparando Email VO com a string do email
    ...
    async def test_update_password_user_not_found(
        ...
        mock_user_repository.update_password.side_effect = ValueError(
        ...
        assert isinstance(called_email_vo, Email)
        assert (
            called_email_vo.value() == non_existent_email_str
        )  # <-- CORREÇÃO: Comparando Email VO com a string do email
